...
class LaserWeaponArmory(Scene):
    ...
    def enter(self):
        ...
        code = f"159"
        guess = input("[keypad]> ")
        # был равен 0 исправил на 1, теперь счетчик считает до 10
        guesses = 1
...
class Restoraunt():
    def enter(self):
        print(dedent("""
            После всего пережитого, ты сидишь в ресторане со своими друзьями.
            По крайней мере так тебе кажется. Со временем ты понимаешь, что среди компании
            есть готом который замаскирован под твоего друга. Тебе нужно его вычислить.
            Как это сделать? имей в виде, что у тебя только 3 попытки.
            """))

        rigth_example = ("Рассказать бородатый анекдот")
        example = input('\n>>>')
        count = 1
        while example != rigth_example and count < 3:
            print("Попробуй еще раз!")
            count += 1
            example = input('\n>>>')
        if example == "Рассказать бородатый анекдот":
            print("Вы и тут одалели этих уродов!!!")
            return 'finished'
